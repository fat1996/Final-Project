...
map<string, int> sblock::updateRows(map<string, int> returnRows, string** board) {
for(int i=0;i<15;i++){
	for(int j=0;j<11;j++){
		cout<<board[i][j];
	}
...
void sblock::drop(map<string, int> returnRows, string** board) {}

void sblock::fillBlock(string** board){
for(int i=0;i<4;i++){
...
blockCoord[0]->setCoord(0, 1);

blockCoord[1]=new Coordinate;
blockCoord[1]->setCoord(0, 2);
